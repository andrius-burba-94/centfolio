...
colors:
  # Light mode
  background: "oklch(0.99 0.005 38)"
  foreground: "oklch(0.22 0.020 38)"
  card: "oklch(0.96 0.010 38)"
  card-foreground: "oklch(0.22 0.020 38)"
  muted: "oklch(0.94 0.012 38)"
  muted-foreground: "oklch(0.50 0.020 38)"
  placeholder: "oklch(0.65 0.020 38)"
  border: "oklch(0.90 0.015 38)"
  input: "oklch(0.80 0.020 38)"
  ring: "oklch(0.55 0.12 38)"
  primary: "oklch(0.45 0.13 38)"
  primary-foreground: "oklch(0.99 0.005 38)"
  primary-hover: "oklch(0.40 0.13 38)"
  primary-active: "oklch(0.35 0.13 38)"
  destructive: "oklch(0.30 0.05 25)"
  destructive-foreground: "oklch(0.99 0.005 38)"
  positive: "oklch(0.30 0.05 130)"
  positive-foreground: "oklch(0.99 0.005 38)"
  # Dark mode
  background-dark: "oklch(0.14 0.012 38)"
  foreground-dark: "oklch(0.95 0.010 38)"
  card-dark: "oklch(0.19 0.014 38)"
  card-foreground-dark: "oklch(0.95 0.010 38)"
  muted-dark: "oklch(0.23 0.014 38)"
  muted-foreground-dark: "oklch(0.75 0.015 38)"
  placeholder-dark: "oklch(0.60 0.020 38)"
  border-dark: "oklch(0.28 0.015 38)"
  input-dark: "oklch(0.38 0.020 38)"
  ring-dark: "oklch(0.65 0.13 38)"
  primary-dark: "oklch(0.65 0.13 38)"
  primary-foreground-dark: "oklch(0.14 0.012 38)"
  primary-hover-dark: "oklch(0.70 0.13 38)"
  primary-active-dark: "oklch(0.75 0.13 38)"
  destructive-dark: "oklch(0.92 0.05 25)"
  destructive-foreground-dark: "oklch(0.14 0.012 38)"
  positive-dark: "oklch(0.92 0.05 130)"
  positive-foreground-dark: "oklch(0.14 0.012 38)"
typography:
  display:
...
Tokens use shadcn's role-named conventions so primitives from the
shadcn ecosystem compose without retokenizing. Custom additions
(`primary-hover`, `primary-active`, `placeholder`, `positive`,
`positive-foreground`) cover roles shadcn does not define.

| Role token | CSS variable | Common Tailwind utility | Use case |
|---|---|---|---|
| `background` | `--background` | `bg-background` | Page background, broadest reading surface |
...
| `ring` | `--ring` | `ring` (default ring color) | Focus rings, selected states, semantic emphasis. The brand terracotta. |
| `primary` | `--primary` | `bg-primary` | Primary button background. Deeper than `ring` in light mode for AAA contrast. |
| `primary-foreground` | `--primary-foreground` | `text-primary-foreground` | Text on `primary`; mode-appropriate page color (cream in light, warm ink in dark) |
| `primary-hover` | `--primary-hover` | `bg-primary-hover` | Primary button hover (deeper in light, lighter in dark) |
| `primary-active` | `--primary-active` | `bg-primary-active` | Primary button pressed |
| `destructive` | `--destructive` | `text-destructive` | Spent ink; form-error text; semantic emphasis |
| `destructive-foreground` | `--destructive-foreground` | `text-destructive-foreground` | Text on `destructive` background (rare) |
| `positive` | `--positive` | `text-positive` | Earned ink. Custom; no shadcn equivalent. |
| `positive-foreground` | `--positive-foreground` | `text-positive-foreground` | Text on `positive` background (rare) |
...
- **Vilnius Brick** (`ring`, `oklch(0.55 0.12 38)` ~ `#B85C3A`):
  the brand identity color in light mode. Used on focus rings, on
  selected states, and on semantic emphasis surfaces. The wordmark
  uses `foreground` (warm near-black), not `ring`, per the
  Publication-Mast Rule.
- **Vilnius Brick (Dark)** (`ring-dark`, `oklch(0.65 0.13 38)` ~
  `#D67A52`): hand-tuned dark-mode counterpart. Same hue and chroma
  family, lightness lifted 0.10 so the accent reads present, not dim,
  against the warm-ink page.

### Primary Button (separate token from brand)

- **Button Brick** (`primary`, `oklch(0.45 0.13 38)` ~ `#9E4928`):
  the Primary button's resting background in light mode. Decoupled
  from `ring`, a deeper terracotta chosen for AAA-adjacent contrast
  against on-button cream text (`primary-foreground`). See the
  Brand-vs-Button Decoupling Rule below.
- **Button Brick (Dark)** uses `primary-dark` (`oklch(0.65 0.13 38)`,
  identical value to `ring-dark` because the dark-mode math works out
  with dark-ink text); on-button text is `primary-foreground-dark`
  (warm ink). Same conceptual frame: the button carves a deeper
  terracotta region from the page and the on-button text reads as the
  page color showing through.
- Hover and active steps move deeper in light (`primary-hover`
  L=0.40, `primary-active` L=0.35) and lighter in dark
  (`primary-hover-dark` L=0.70, `primary-active-dark` L=0.75). The
  semantic (feedback through contrast shift) holds; the direction
  adapts to mode.

### Neutral (light mode)

- **Warm Paper** (`background` `oklch(0.99 0.005 38)` ~ `#FCF9F6`):
  the broadest reading surface. Brightest token in the system. The
  page IS the paper.
- **Tinted Block** (`card` `oklch(0.96 0.010 38)` ~ `#F2EAE2`):
  cards, content containers, toast bodies. Slightly more chromatic
  than the page so the block reads as set into it.
- **Tinted Block Deep** (`muted` `oklch(0.94 0.012 38)` ~ `#EBE0D5`):
  hover backgrounds, initials-circle background, ghost-button-active
  background. The third step in the layering ramp.
- **Subtle Border** (`border` `oklch(0.90 0.015 38)` ~ `#DDD0C2`):
  barely-there dividers, table rows.
- **Strong Border** (`input` `oklch(0.80 0.020 38)` ~ `#C2B0A0`):
  inputs at rest, visible dividers.
- **Ink** (`foreground` `oklch(0.22 0.020 38)` ~ `#2A1F18`):
  warm near-black. The reading text. AAA-clears against `background`
  at roughly 12:1.
- **Ink Secondary** (`muted-foreground` `oklch(0.50 0.020 38)` ~
  `#6E5848`): chrome-adjacent copy. AA against `background`.
- **Ink Tertiary** (`placeholder` `oklch(0.65 0.020 38)` ~
  `#95826F`): placeholders, disabled. AA against `background`.
...
- **Warm Ink Page** (`background-dark` `oklch(0.14 0.012 38)` ~
  `#221710`): the warm-ink page.
- **Lifted Block** (`card-dark` `oklch(0.19 0.014 38)` ~ `#2F2218`):
  cards. Slightly lifted from the page.
- **Lifted Block Deep** (`muted-dark` `oklch(0.23 0.014 38)` ~
  `#3A2C22`): hovered or nested surfaces.
- **Subtle Border (Dark)** (`border-dark` `oklch(0.28 0.015 38)` ~
  `#463629`): barely-there dividers.
- **Strong Border (Dark)** (`input-dark` `oklch(0.38 0.020 38)` ~
  `#5C4836`): inputs at rest, visible borders.
- **Cream Ink** (`foreground-dark` `oklch(0.95 0.010 38)` ~
  `#F3EBE3`): the reading text in dark mode. AAA-clears against
  `background-dark` at roughly 14:1.
- **Cream Ink Secondary** (`muted-foreground-dark`
  `oklch(0.75 0.015 38)` ~ `#BFA993`): chrome-adjacent.
  AAA-adjacent against `background-dark`.
- **Cream Ink Tertiary** (`placeholder-dark` `oklch(0.60 0.020 38)` ~
  `#957C66`): placeholders, disabled.
...
- **Spent Ink** (`destructive` `oklch(0.30 0.05 25)` ~ `#5C2E27`):
...
- **Earned Ink** (`positive` `oklch(0.30 0.05 130)` ~ `#324832`):
  the slight green-tinted warm-near-black for amounts that come in.
- **Spent Ink (Dark)** (`destructive-dark` `oklch(0.92 0.05 25)` ~
  `#EFCBC0`) and **Earned Ink (Dark)** (`positive-dark`
...
- `foreground`: body copy, numerals, field labels, anything
  load-bearing for comprehension. AAA contrast required against any
  background.
- `muted-foreground`: captions, tooltips, helper text, metadata
  *adjacent to* a primary value. AA floor; AAA where the palette
  allows. Never used for numerals or amounts.
- `placeholder`: placeholders, disabled states, decorative
  timestamps. AA target; sub-AA acceptable for placeholders only.
  Never used for content the user must read.

The drift this rule prevents: reaching for `muted-foreground` on
"less important but still meaningful" labels. A caption like
*"vs. last month"* tucked under a numeral is load-bearing for
comprehension even when it is visually subordinate. That label uses
`foreground` at a smaller size, not `muted-foreground` at full size.

**The Sign-First Rule.** Direction-of-money is carried by **sign,
label, or position**; color reinforces. `destructive` and `positive`
are never the sole indicator. A spent value may read as `−€34.27`
with `destructive` tint (sign), or as a row labeled "Spent: €34.27"
...
the `background → card → muted` ramp, not from `box-shadow`. This
honors the editorial / publication positive reference: magazines
convey depth through type, color, and layout, not through
...
- **Resting:** `primary` background (light) / `primary-dark`
  background (dark); on-button text is `primary-foreground`
  (mode-appropriate page color: cream in light, warm ink in dark).
- **Padding:** 12px 20px.
- **Hover:** background steps to `primary-hover` in light (deeper) or
  `primary-hover-dark` in dark (lighter).
- **Active:** background steps further to `primary-active` /
  `primary-active-dark`.
- **Focus:** see Mode-Appropriate Focus Ring Rule.
- **Disabled:** background `input`; text `placeholder`; cursor
  `not-allowed`.

**Ghost**
- **Shape:** `rounded-sm`.
- **Resting:** transparent background; `muted-foreground` for label.
...
- **Background:** `background`, even when the input lives inside a
  card. The Cut-Into-the-Page Rule is load-bearing.
- **Border:** 1px solid `input`.
- **Shape:** `rounded-sm`.
- **Padding:** 10px 14px.
- **Typography:** body (Inter 400, 17px). `foreground` for value;
  `placeholder` for placeholder.
- **Focus:** border swaps to `ring` (mode-appropriate) plus a 2px
  mode-appropriate ring. No background shift.
- **Error:** border swaps to `destructive` (mode-appropriate). Helper
  text below the input renders at label size in `destructive`.
- **Disabled:** background `card`; text `placeholder`; cursor
  `not-allowed`.
...
uses label typography in `destructive`, positioned 6px below the
input.
...
- **Background:** `card`.
- **Border:** none at rest. 1px `border` on hover/focus *only if the
  card is interactive*. Phase 1 dashboard cards are not interactive.
...
- **default**: no icon; `foreground` content.
- **success**: 16px checkmark in mode-appropriate `positive` +
  `foreground` content.
- **error**: 16px exclamation in mode-appropriate `destructive` +
  `foreground` content. Duration extended to 6000ms (more reading
  time).
...
- **Weight:** 400 (Regular).
- **Size:** 22px.
- **Color:** `foreground`, **not** `ring` (terracotta).
- **Tracking:** -0.005em.
...
- **Trigger:** Ghost button, 32×32px, square padding. Icon 18px
  ...
  in `muted-foreground`.
- **Dropdown:** 8px gap below the trigger, right-aligned. `card`
  background, `rounded-md`, `lifted-floating` shadow, 8px padding.
- **Dropdown item:** 8px 12px padding, body typography, `foreground`.
  Hover background `muted`. Current selection marked with a
  mode-appropriate `ring` checkmark on the right.
...
- **Trigger (initials circle):**
  ...
  - Focus: 2px mode-appropriate `ring`, 2px offset.
- **Dropdown:** identical structural treatment to the theme-toggle
  dropdown (right-aligned, `card`, `lifted-floating` shadow,
...
**The Cut-Into-the-Page Rule.** Inputs sit on `background`, never on
`card`, even when the form lives inside a card. This inverts the SaaS
convention (bright input on dim background) and ties to the editorial
framing: the page is the writing surface; an input is a space the
user writes back into it. If a form lives in a card, the card uses
`card` but the input's background remains `background`. The input
visually breaks through the card to the page beneath.

**The Brand-vs-Button Decoupling Rule.** The brand terracotta
(`ring`, L=0.55, the system's identity color) carries focus rings,
selected states, and semantic emphasis: surfaces where it has no
on-color text contrast obligation. The Primary button background is
a *separate* token (`primary`, L=0.45 in light mode) chosen for
AAA-adjacent contrast against on-button text. Same hue family,
different lightness. Do not conflate the two: setting
`bg-primary = var(--ring)` directly fails AA on cream text and
violates the Reading-Load Rule on the system's most action-critical
UI surface.
...
- color: `ring` in light mode, `ring-dark` in dark mode, declared as
  a CSS custom property (`--ring`) so Tailwind's `@custom-variant
  dark` swap applies automatically
...
in a `muted`-background circle. No Avatar primitive, no image
upload, no placeholder photo. Initials in a tinted circle is three
Tailwind classes; the Avatar primitive's variants (image, fallback,
sizes, groups) earn their keep only when at least one of those
variants is needed. They are not, yet.
...
- **Do** use `background` as the input background, even when the
  input lives inside a card (Cut-Into-the-Page Rule).
- **Do** carry direction-of-money via sign, label, or position
  first; color reinforces (Sign-First Rule).
- **Do** reserve `ring` (the brand terracotta, L=0.55) for focus
  rings, selected states, and semantic emphasis. The wordmark uses
  `foreground`, not `ring` (per the Publication-Mast Rule). Primary
  button background is the separate `primary` token (L=0.45) (per
  the Brand-vs-Button Decoupling Rule).
...
- **Don't** use `muted-foreground` on numerals or amounts
  (Reading-Load Rule).
- **Don't** set `bg-primary = var(--ring)` directly
